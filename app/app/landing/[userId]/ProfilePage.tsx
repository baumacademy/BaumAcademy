...
const Profile: React.FC<Props> = ({ user }) => {
    ...
    const saveChanges = async () => {
        ...
        const response = await axios.patch(`${apiURL}/${user?.id}`, profileData)
        setIsEditing(false);
    };
    ...
    return (
        <div className="flex flex-col items-center bg-gray-100 p-6 rounded-lg shadow-md max-w-lg mx-auto my-10 min-w-fit">
            ...
            {isEditing ? (
                ...
                <>
                    <h2 className="mt-4 text-2xl font-semibold text-gray-800">{profileData?.firstName + " " + profileData?.lastName}</h2>
                </>
            )}
                ...
                {isEditing ? (
                    ...
                    <>
                        ...
                        <p className="text-gray-600">
                            <span className="font-semibold">Location:</span> {profileData?.city ?? 'N/A'}
                        </p>
                        ...
                        <p className="text-gray-600">
                            <span className="font-semibold">Occupation:</span> {profileData?.occupation ?? 'N/A'}
                        </p>
                        <p className="text-gray-600">
                            <span className="font-semibold">Batch #:</span> {profileData?.batch ?? 'N/A'}
                        </p>
                    </>
                )}
                {isEditing ? <>
                    ...
                    <span className="font-semibold">Enrolled Course:</span> {profileData?.class?.subject}
                </p>
                    <p className="text-gray-600">
                        <span className="font-semibold">Course Description:</span> {profileData?.class?.content}
                    </p></>}
            </div>
